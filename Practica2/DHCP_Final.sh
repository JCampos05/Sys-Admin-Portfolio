...
monitoreo_info(){
    ...
    # Concesiones activas (solo IPs únicas más recientes)
    echo ""
    echo "Concesiones activas:"
    echo "--------------------"
    if [ -f /var/lib/dhcpd/dhcpd.leases ]; then
        # Extraer solo las IPs únicas más recientes
...
pre_main(){
    ...
    if ! rpm -q dhcp-server &> /dev/null; then
        ...
        deteccion_interfaces_red
        echo "prosiguiendo al monitoreo..."
        return 0    
    fi
}
