...
function parametros_usuario {
    ...
    while ($true) {
        Write-Host ""
        $script:dns = Read-Host "IP del servidor DNS"
...
function config_dhcp {
    ...
    $existingScope = Get-DhcpServerv4Scope -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq $script:nombreScope }
    ...
    try {
        ...
        Write-Host "Opciones de red configuradas correctamente" 
    } catch {
        Write-Host "Error al configurar opciones: $_" 
        exit 1
    }
...
function main {
    ...
    # Monitoreo continuo con actualizacion cada 5 segundos
    while ($true) {
        Clear-Host
        monitoreo_info
