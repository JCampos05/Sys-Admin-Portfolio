#
# FunctionsFTP-B.ps1
# Grupo B — Instalacion y desinstalacion del servidor FTP
#
# Requiere: utilsFTP.ps1, subMainFTP.ps1, FunctionsFTP-C.ps1, FunctionsFTP-D.ps1
#
...
function Install-FtpServer {
    ...
    New-FtpDirectoryStructure   # definida en 4.directorios.ps1
    Init-FtpMeta
    New-FtpSite
...
function Menu-Instalacion {
    ...
    while (-not $salir) {
        Clear-Host
        Draw-Header "Instalacion del Servidor FTP"
        Write-Host "  1) Instalar servidor FTP (IIS + FTP + User Isolation)"
        Write-Host "  2) Desinstalar servidor FTP"
        ...
        switch ($op) {
            ...
            "2" {
                if (Confirm-MenuAction "Esto eliminara el sitio FTP. Continuar?") {
                    Uninstall-FtpServer
                }
                Pause-Menu
            }
            "3" { $salir = $true }
            default {
                msg_error "Opcion invalida. Seleccione del 1 al 3"
