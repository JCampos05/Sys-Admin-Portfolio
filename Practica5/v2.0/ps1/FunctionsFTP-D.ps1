#
# FunctionsFTP-D.ps1
# Grupo D — Directorios NTFS, permisos y junctions
#
# Requiere: utilsFTP.ps1, subMainFTP.ps1
#
...
function New-FtpDirectoryStructure {
    ...
    Set-NtfsPermission $script:FTP_ROOT "BUILTIN\Administrators" "FullControl" | Out-Null
    Set-NtfsPermission $script:FTP_ROOT "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null

    # LocalUser
    Disable-NtfsInheritance "$script:FTP_ROOT\LocalUser"
    Set-NtfsPermission "$script:FTP_ROOT\LocalUser" "BUILTIN\Administrators" "FullControl" | Out-Null
    Set-NtfsPermission "$script:FTP_ROOT\LocalUser" "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
    ...
    Set-NtfsPermission $script:FTP_GENERAL "BUILTIN\Administrators" "FullControl" | Out-Null
    Set-NtfsPermission $script:FTP_GENERAL "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
    Set-NtfsPermission $script:FTP_GENERAL $script:FTP_GROUP_ALL    "Modify" | Out-Null
    Set-NtfsPermission $script:FTP_GENERAL "IUSR"                   "ReadAndExecute" | Out-Null
    # FIX: Deny explícito para IUSR — gana sobre cualquier Allow heredado de grupo
    Set-NtfsPermission $script:FTP_GENERAL "IUSR" "Write,Delete,DeleteSubdirectoriesAndFiles,CreateFiles,CreateDirectories" "Deny" | Out-Null
    ...
    Set-NtfsPermission "$script:FTP_ROOT\LocalUser" "IUSR" "ReadAndExecute" "Allow" "None" "None" | Out-Null

    # Carpetas de grupo
    foreach ($grupo in $script:FTP_GROUPS) {
        $dir = "$script:FTP_ROOT\grupos\$grupo"
        Disable-NtfsInheritance $dir
        Set-NtfsPermission $dir "BUILTIN\Administrators" "FullControl" | Out-Null
        Set-NtfsPermission $dir "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
        Set-NtfsPermission $dir $grupo                   "Modify" | Out-Null
    }
...
function New-FtpUserDirectories {
    ...
    Set-NtfsPermission $userDir "BUILTIN\Administrators" "FullControl" | Out-Null
    Set-NtfsPermission $userDir "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
    Set-NtfsPermission $userDir $usuario "ReadAndExecute" "Allow" "None" "None" | Out-Null

    # Personal: control total dentro de la carpeta
    Disable-NtfsInheritance $personal
    Set-NtfsPermission $personal "BUILTIN\Administrators" "FullControl" | Out-Null
    Set-NtfsPermission $personal "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
    Set-NtfsPermission $personal $usuario                 "Modify" | Out-Null
        ...
        Get-Content $script:FTP_META | ForEach-Object {
            if ($_ -match '^(.+):(.+)$') {
                ...
                if (Test-Path $userDir) {
                    Set-NtfsPermission $userDir  "BUILTIN\Administrators" "FullControl" | Out-Null
                    Set-NtfsPermission $userDir  "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
                    Set-NtfsPermission $userDir  $u "ReadAndExecute" "Allow" "None" "None" | Out-Null
                    msg_success "$userDir reparado"
                }
                if (Test-Path $personal) {
                    Set-NtfsPermission $personal "BUILTIN\Administrators" "FullControl" | Out-Null
                    Set-NtfsPermission $personal "NT AUTHORITY\SYSTEM"    "FullControl" | Out-Null
                    Set-NtfsPermission $personal $u                       "Modify" | Out-Null
