...
#
# Propósito del grupo:
#   C.1 — Cambio de puerto (edge case completo con rollback)
#   C.2 — Security headers (ServerTokens, X-Frame-Options, X-Content-Type-Options)
#   C.3 — Control de métodos HTTP (TRACE, TRACK, DELETE, PUT)
#   C.4 — Submenú integrador + acceso al Grupo D
#
# Requiere:
#   utils.sh, utils_http.sh, validators_http.sh
#   http_functions_A.sh (http_verificar_respuesta, _http_obtener_puerto_activo)
#   http_functions_B.sh (_http_configurar_puerto_inicial, http_crear_index)
#
...
_http_seleccionar_servicio_instalado() {
    ...
    if [[ ${#servicios_instalados[@]} -eq 0 ]]; then
        aputs_error "No hay ningun servicio HTTP instalado en el sistema"
        aputs_info  "Vaya a la opcion 2) Instalar servicio HTTP"
        return 1
    fi
    ...
    local opcion
    while true; do
        agets "Seleccione el servicio [1-${#servicios_instalados[@]}]" opcion
...
_http_actualizar_firewall_puerto() {
    ...
    aputs_info "Actualizando reglas de firewall..."
    echo ""
    ...
    if ! sudo firewall-cmd --list-ports 2>/dev/null \
        ...
        if sudo firewall-cmd --permanent \
            ...
            aputs_success "Puerto ${puerto_nuevo}/tcp abierto (permanente)"
        else
            aputs_error "No se pudo abrir el puerto ${puerto_nuevo}/tcp"
            return 1
        fi
    else
        aputs_info "Puerto ${puerto_nuevo}/tcp ya estaba abierto"
    fi
    ...
    if ! http_puerto_en_uso "$puerto_viejo"; then
        if (( puerto_viejo == 80 )); then
            sudo firewall-cmd --permanent --remove-service=http \
                 2>/dev/null \
            && aputs_success "Servicio http (80) eliminado del firewall"
        fi
        if sudo firewall-cmd --list-ports 2>/dev/null \
           | grep -q "${puerto_viejo}/tcp"; then
            sudo firewall-cmd --permanent \
                 --remove-port="${puerto_viejo}/tcp" 2>/dev/null \
            && aputs_success "Puerto ${puerto_viejo}/tcp cerrado en firewall"
        fi
    else
        local proceso_viejo
        proceso_viejo=$(http_quien_usa_puerto "$puerto_viejo")
        aputs_info "Puerto ${puerto_viejo} aun en uso por '${proceso_viejo}' — no se cierra"
    fi
...
http_cambiar_puerto() {
    ...
    if [[ -z "$puerto_actual" ]]; then
        ...
        aputs_info "Usando puerto por defecto: ${puerto_actual}"
    else
        aputs_info "Puerto actual configurado: ${puerto_actual}/tcp"
    fi
    ...
    local puerto_nuevo
    while true; do
        agets "Nuevo puerto [actual: ${puerto_actual}]" puerto_nuevo
    ...
    local confirmacion
    while true; do
        agets "Confirmar cambio? [s/n]" confirmacion
        http_validar_confirmacion "$confirmacion"
        local rc=$?
        (( rc == 0 )) && break
        (( rc == 1 )) && { aputs_info "Cambio cancelado"; sleep 1; return 0; }
        echo ""
    done
    ...
    aputs_info "PASO 2/5 — Aplicando nuevo puerto en ${archivo_conf}"
    ...
    aputs_info "PASO 4/5 — Verificando respuesta HTTP en puerto ${puerto_nuevo}"
    sleep 2

    if ! http_verificar_respuesta "$servicio" "$puerto_nuevo"; then
        aputs_error "Sin respuesta en puerto ${puerto_nuevo} — restaurando"
        http_restaurar_backup "$archivo_conf"
        http_reiniciar_servicio "$servicio"
        aputs_info "Configuracion restaurada al puerto ${puerto_actual}"
        return 1
    fi
    echo ""

    # Paso 8: Firewall
    aputs_info "PASO 5/5 — Actualizando reglas de firewall"
    ...
    echo ""
    draw_line
    aputs_success "Puerto cambiado exitosamente: ${puerto_actual} -> ${puerto_nuevo}"
    draw_line
}
...
_http_seguridad_apache() {
    aputs_info "Aplicando security headers en Apache..."
    echo ""
...
APACHEEOF
    ...
    if [[ $? -eq 0 ]]; then
        aputs_success "security.conf escrito"
        ...
        aputs_error "No se pudo escribir security.conf"
        return 1
    fi
}
...
_http_seguridad_nginx() {
    ...
    _nginx_set_header() {
        ...
        if sudo grep -q "add_header ${nombre}" "$HTTP_CONF_NGINX" 2>/dev/null; then
            ...
            aputs_success "${nombre}: actualizado"
        else
            sudo sed -i "/server_tokens off;/a\\${directiva}" "$HTTP_CONF_NGINX"
            aputs_success "${nombre}: agregado"
        fi
    }
    ...
    if sudo nginx -t 2>/dev/null; then
        ...
        aputs_error "Error de sintaxis — restaurando backup"
...
_http_seguridad_tomcat() {
    ...
    if [[ ! -f "$webxml" ]]; then
        aputs_error "web.xml no encontrado: ${webxml}"
        return 1
    fi
    ...
    if [[ -z "$linea_cierre" ]]; then
        aputs_error "Estructura web.xml invalida — falta </web-app>"
...
_http_metodos_apache() {
    ...
    aputs_info "Configurando metodos HTTP en Apache..."
    echo ""
...
EOF
    ...
    aputs_success "Metodos permitidos en Apache: ${metodos_permitidos}"
    return 0
}
...
_http_metodos_nginx() {
    ...
    aputs_info "Configurando metodos HTTP en Nginx..."
    echo ""
    ...
    if sudo nginx -t 2>/dev/null; then
        aputs_success "Metodos bloqueados en Nginx: ${metodos_bloqueados}"
        return 0
    else
...
_http_metodos_tomcat() {
    ...
    aputs_info "Configurando metodos HTTP en Tomcat..."
    echo ""
    ...
    if [[ -z "$linea_cierre" ]]; then
        aputs_error "web.xml invalido — falta </web-app>"
        http_restaurar_backup "$webxml"
        return 1
    fi
...
$(printf "%b" "$metodos_xml")    </web-resource-collection>\\
    ...
    aputs_success "Metodos bloqueados en Tomcat: ${metodos_bloqueados_str}"
    return 0
}
...
http_restringir_metodos() {
    ...
    local perfil
    while true; do
        agets "Seleccione perfil [1-3]" perfil
    ...
    case "$perfil" in
        ...
        3)
            ...
            aputs_info "Ingrese los metodos a bloquear separados por espacio (MAYUSCULAS)"
            echo ""
            local entrada_metodos
            agets "Metodos a bloquear" entrada_metodos
    ...
    local confirmacion
    while true; do
        agets "Confirmar? [s/n]" confirmacion
        http_validar_confirmacion "$confirmacion"
        local rc=$?
        (( rc == 0 )) && break
        (( rc == 1 )) && { aputs_info "Operacion cancelada"; sleep 1; return 0; }
        echo ""
    done
...
http_menu_configurar() {
    while true; do
        ...
        case "$op" in
            1) http_cambiar_puerto;      echo ""; pause ;;
            2) http_configurar_seguridad; echo ""; pause ;;
            3) http_restringir_metodos;  echo ""; pause ;;
            4) http_menu_versiones ;;
            5) return 0 ;;
            *) aputs_error "Opcion invalida. Seleccione entre 1 y 5"; sleep 2 ;;
        esac
    done
}
