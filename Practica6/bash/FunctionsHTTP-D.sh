...
# http_functions_D.sh
#
#   Administrar el ciclo de vida de versiones de un servicio ya instalado
#   sin necesidad de desinstalarlo y volverlo a instalar manualmente.
#
#   D.1 — Consultar versión instalada actualmente
#         Muestra: versión rpm, fecha de instalación, puerto activo,
#         estado del servicio y si existe una versión más reciente disponible.
#
#   D.2 — Upgrade (actualización a versión superior)
#         Consulta versiones disponibles en dnf más recientes que la actual.
#         Instala con 'dnf upgrade' de forma silenciosa.
#         Preserva el puerto y la configuración de seguridad activos.
#         Verifica respuesta HTTP tras el upgrade.
#
#   D.3 — Downgrade (retroceso a versión anterior)
#         Consulta versiones disponibles inferiores a la actual.
#         Instala con 'dnf downgrade' de forma silenciosa.
#         Mismo proceso de preservación y verificación que el upgrade.
#
#   D.4 — http_menu_versiones — punto de entrada desde http_menu_configurar
#
# Funciones públicas:
#   http_ver_version_instalada()  — Panel de versión y estado actual
#   http_upgrade_servicio()       — Actualización a versión superior
#   http_downgrade_servicio()     — Retroceso a versión anterior
#   http_menu_versiones()         — Submenú del Grupo D
#
# Funciones internas (prefijo _):
#   _http_versiones_superiores()  — Filtra versiones dnf > actual
#   _http_versiones_inferiores()  — Filtra versiones dnf < actual
#   _http_comparar_versiones()    — Compara dos versiones semver
#   _http_ejecutar_cambio_version() — Orquestador upgrade/downgrade común
#
# Requiere:
#   utils.sh, utils_http.sh, validators_http.sh
#   http_functions_A.sh (_http_obtener_puerto_activo, http_verificar_respuesta)
#   http_functions_B.sh (http_consultar_versiones, _http_instalar_paquete,
#                        http_crear_index, _http_habilitar_servicio)
#   http_functions_C.sh (_http_seleccionar_servicio_instalado,
...
http_ver_version_instalada() {
    ...
    for i in "${!servicios[@]}"; do
        ...
        aputs_info "  Consultando ultima version disponible en repositorios..."
...
_http_ejecutar_cambio_version() {
    ...
    if (( total == 0 )); then
        if [[ "$operacion" == "upgrade" ]]; then
            aputs_info "No hay versiones superiores disponibles en los repositorios"
            aputs_info "El servicio ya esta en la version mas reciente"
        else
            aputs_info "No hay versiones anteriores disponibles en los repositorios"
    ...
    aputs_info "Version actual    : ${version_actual}"
    aputs_info "Puerto preservado : ${puerto_actual}/tcp"
    echo ""
    ...
    aputs_info "Versiones ${etiqueta_op} disponibles en repositorios:"
    echo ""
    printf "  %-5s %-35s\n" "NUM" "VERSION"
    echo "  ──────────────────────────────────────────"
    ...
    while true; do
        agets "Seleccione version destino [1-${total}]" indice_elegido
    ...
    local confirmacion
    while true; do
        agets "Confirmar ${operacion}? [s/n]" confirmacion
    ...
    aputs_info "PASO 2/5 — Ejecutando dnf ${operacion} a ${version_destino}"
    echo ""
    ...
    if [[ "$version_nueva" == "$version_actual" ]]; then
        ...
        aputs_info "dnf puede haber omitido el cambio si ya estaba satisfecho"
        aputs_info "Version reportada: ${version_nueva}"
    else
        aputs_success "Version actualizada: ${version_actual} → ${version_nueva}"
    fi
    ...
    aputs_info "PASO 4/5 — Reaplying puerto ${puerto_actual} en configuracion"
    ...
    if ! http_reiniciar_servicio "$servicio"; then
        ...
        aputs_info "Intentando restaurar version ${version_actual}..."
    ...
    aputs_info "Verificando respuesta HTTP en puerto ${puerto_actual}..."
    sleep 2

    if ! http_verificar_respuesta "$servicio" "$puerto_actual"; then
        aputs_warning "El servicio no responde — puede necesitar mas tiempo"
        aputs_info "Verifique manualmente: curl -I http://localhost:${puerto_actual}"
    fi
    ...
    echo ""
    draw_line
    aputs_success "${operacion^} completado: ${version_actual} → ${version_nueva}"
...
http_upgrade_servicio() {
    ...
    aputs_info "los repositorios de Fedora. Preserva el puerto configurado."
    echo ""
    ...
    aputs_info "Consultando versiones superiores disponibles..."
    echo ""
    ...
    if ! _http_versiones_superiores "$servicio" "$version_actual" \
        ...
        aputs_error "No se pudo obtener versiones del repositorio"
        return 1
    fi
...
http_downgrade_servicio() {
    ...
    aputs_info "Consultando versiones anteriores disponibles..."
    echo ""
