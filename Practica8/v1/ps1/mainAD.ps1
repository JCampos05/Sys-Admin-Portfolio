...
function _estado_logonhours {
    ...
    try {
        $primerUsuario = (Get-ADGroupMember "GRP_Cuates" -ErrorAction Stop |
                          Where-Object { $_.objectClass -eq "user" } |
                          Select-Object -First 1).SamAccountName
        $bytes = (Get-ADUser $primerUsuario -Properties LogonHours -ErrorAction Stop).LogonHours
...
function _menu_clientes {
    while ($true) {
        ...
        if ($hora -ge 8 -and $hora -lt 15) {
            $grupoActivo = "${GREEN}GRP_Cuates${NC} (8AM-3PM)"
        } elseif ($hora -ge 15 -or $hora -lt 2) {
            $grupoActivo = "${GREEN}GRP_NoCuates${NC} (3PM-2AM)"
        } else {
        ...
        switch ($op) {
            ...
            "5" {
                ...
                if ($confirm -eq "SI") {
                    foreach ($sam in (Get-ADUser -Filter * -SearchBase (Get-DomainNC -DomainName (Get-ADDomain).DNSRoot) -Properties Department |
...
function _menu_verificacion {
    ...
    # LogonHours — usar primer usuario real de cada grupo
    Write-Host ""
    aputs_info "-- LogonHours --"
    Write-Host ""
    $samCuates   = (Get-ADGroupMember "GRP_Cuates"   -EA SilentlyContinue |
                    Where-Object { $_.objectClass -eq "user" } |
                    Select-Object -First 1).SamAccountName
    $samNoCuates = (Get-ADGroupMember "GRP_NoCuates" -EA SilentlyContinue |
                    Where-Object { $_.objectClass -eq "user" } |
                    Select-Object -First 1).SamAccountName

    foreach ($sam in @($samCuates, $samNoCuates) | Where-Object { $_ }) {
        try {
    ...
    # FSRM — usar primer usuario real de cada grupo
    Write-Host ""
    aputs_info "-- FSRM --"
    Write-Host ""
    ...
    if ($samCuates) {
        try {
            $q1 = Get-FsrmQuota "C:\Perfiles\$samCuates" -EA Stop
            _v_check "Cuota $samCuates (Cuates)" "ok" "$([Math]::Round($q1.Size/1MB,0)) MB Hard"
        } catch { _v_check "Cuota $samCuates" "fail" "(no encontrada)" }
    }
    if ($samNoCuates) {
        try {
            $q6 = Get-FsrmQuota "C:\Perfiles\$samNoCuates" -EA Stop
            _v_check "Cuota $samNoCuates (NoCuates)" "ok" "$([Math]::Round($q6.Size/1MB,0)) MB Hard"
        } catch { _v_check "Cuota $samNoCuates" "fail" "(no encontrada)" }
    }
    if ($samCuates) {
        try {
            $fs   = Get-FsrmFileScreen "C:\Perfiles\$samCuates" -EA Stop
